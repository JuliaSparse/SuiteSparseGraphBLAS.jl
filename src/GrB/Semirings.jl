...
import ..GrB: isGxB, isGrB, Utypes,
    builtin_vec, symtotype, Itypes, Ftypes, Ztypes, FZtypes, IFtypes,
    ...
    GB_promote, GxB_fprint, defaultmonoid,
    Monoid, BinaryOp, load_global, LibGraphBLAS, gxbprint, domains, Semiring

import SparseBase: storedeltype
export Semiring

domains(::Semiring{X, Y, Z}) where {X, Y, Z} = (X, Y) => Z
...
function Semiring(addop, mulop, ::Type{T}, ::Type{U}, ::Type{V}) where {T, U, V}
    ...
    return Base.get!(SEMIRINGS, (monoid, binop, T, U)) do
