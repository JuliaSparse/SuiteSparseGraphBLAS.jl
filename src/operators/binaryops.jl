...
mutable struct BinaryOp{F, F2, X, Y, Z}
    ...
    function BinaryOp{F, X, Y, Z}(
        ...
        return finalizer(binop) do op
            if op.loaded
                GrB.@checkfree LibGraphBLAS.GrB_BinaryOp_free(Ref(op.p))
            end
        end
    end
end
...
function Base.unsafe_convert(::Type{LibGraphBLAS.GrB_BinaryOp}, op::BinaryOp{F, F2, X, Y, Z}) where {F, F2, X, Y, Z}
    if !op.loaded
        if op.builtin
            ...
            z, x, y = GrB_Type(Z), GrB_Type(X), GrB_Type(Y)
...
function BinaryOp(
    ...
    maybeop = get!(BINARYOPS, (f, x, y, z)) do
